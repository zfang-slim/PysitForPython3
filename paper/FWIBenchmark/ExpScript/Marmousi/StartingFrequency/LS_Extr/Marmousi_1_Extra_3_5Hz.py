...
if __name__ == '__main__':
    ...
    for exp_ind in range(len(ALPHA_ALL)-1, len(ALPHA_ALL)):
        ...
        SubDir = '/LS_Starting3_5HzExtraDownSample'
        Datafile = '/wavedata/hongyus/ToZhilong/fullband_downsample/extradata_3_5Hz_batch4.mat'
    ...
    #   comm = MPI.COMM_SELF
        size = comm.Get_size()
        rank = comm.Get_rank()
        n_subshot = Nshots // size

        A = sio.loadmat(Datafile)
        Data = A['shot_extra']
        Datai = Data[:, nrec*n_subshot*rank:nrec*n_subshot*(rank+1)]
        ...
        for isrc in range(n_subshot):
            shots[isrc].receivers.reset_time_series(solver.ts()[0::downratio])
            shots[isrc].receivers.data = Datai[:,nrec*isrc:nrec*(isrc+1)]
        ...
        for i_freq in range(len(freq_bands)):
            ...
            filter_op = band_pass_filter(nt_obs, solver.tf, freq_band, transit_freq_length=0.5, padding_zeros=True, nl=500//downratio, nr=500//downratio)
            # filter_op = None
            ...
            objective = TemporalLeastSquares(solver, filter_op=filter_op, parallel_wrap_shot=pwrap, DownSample_op=DownSampleOp)
           ...
           # Proj_Op = JointPrj(Proj_Op1, Boxproj)
            ...
            invalg = PQN(objective, proj_op=Proj_Op, memory_length=10, maxiter_PGD=1000, PQN_start_iteration=0)
            ...
            result = invalg(shots, initial_value, nsteps[i_freq],
