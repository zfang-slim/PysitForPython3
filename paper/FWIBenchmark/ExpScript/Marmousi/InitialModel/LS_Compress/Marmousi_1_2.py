...
if __name__ == '__main__':
    ...
    for exp_ind in range(len(ALPHA_ALL)-1, len(ALPHA_ALL)):
        ...
        RootDir = '/scratch/Zhilong/ExxonProject/MarmousiModel/Result/ExpPeriod201909_2/InitialModel/LS_Compress_Result'
        WaveRootDir = '/wavedata/Zhilong/ExxonProject/MarmousiModel/Result/ExpPeriod201909_2/InitialModel/LS_Compress_Result'
        # SubDir = '/OT_Hard_PML50_Initial00_4s_3_7Hz'
        SubDir = '/LS_BoxConstraint_SmoothInitial500_CorrectBox2'
    ...
    #   comm = MPI.COMM_SELF
        ...
        nktime_slice = 80
        rank_app_tensor = np.array([3,10,5]).astype(int)
        addrank = np.array([0.34, 10, 5])
        tensor_shape = [nktime_slice, m_shape[0], m_shape[1]]
        CompWaveInfo = CompressWaveInfor(rank_app_tensor,addrank, tensor_shape, relerr=2e-1)
        ...
        for i_freq in range(len(freq_bands)):
            ...
            objective = TemporalLeastSquares(solver, WaveCompressInfo=CompWaveInfo, filter_op=filter_op, parallel_wrap_shot=pwrap, normalize_trace=False)
