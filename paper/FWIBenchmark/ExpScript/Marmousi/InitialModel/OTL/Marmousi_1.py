...
if __name__ == '__main__':
    ...
    nsteps = 100 # number of iteration per frquency band
    ...
    for exp_ind in range(len(ALPHA_ALL)-1, len(ALPHA_ALL)):
        ...
        SubDir = '/OTL_BoxConstraint_SmoothInitial500_CorrectBox'
    ...
    #   comm = MPI.COMM_SELF
        ...
        for i in range(len(shots)):
            ...
            # tolerance_ranki += 0.5*np.linalg.norm(noise_i.flatten())**2.0

        # if pwrap.use_parallel:
	       #  # Allreduce wants an array, so we give it a 0-D array
	       #  new_tolerance = np.array(0.0)
	       #  pwrap.comm.Allreduce(np.array(tolerance_ranki), new_tolerance)
	       #  tolerance = new_tolerance[()]  # goofy way to access 0-D array element
        ...
        if rank == 0:
            ...
            for i_freq in range(n_fband):
                ...
                for k in range(int(n_iter_used[i_freq])):
            ...
            write_data(WaveDir + '/Dobs_clean.mat', shots_clean[0].receivers.data, odata, ddata, ndata)
            write_data(ExpDir + '/Dobs_clean.mat', shots_clean[0].receivers.data, odata, ddata, ndata)
