...
if __name__ == '__main__':
    ...
    nsteps_OTQ = 50 # number of iteration per frquency band
    nsteps_LS = 50
    freq_bands = [[3.5,7.5]] # frequency bands used in the inversion
    ...
    for exp_ind in range(len(ALPHA_ALL)-1, len(ALPHA_ALL)):
        ...
        SubDir = '/OTQ_BoxConstraint_SmoothInitial500_CorrectBox'
    ...
    #   comm = MPI.COMM_SELF
        ...
        for i_freq in range(len(freq_bands)):
            dir_ifreq = 'xm'+str(i_freq+1)+'_OTQ'
            ...
            # objective = TemporalOptimalTransport(solver, filter_op=filter_op, parallel_wrap_shot=pwrap)
            objective = TemporalOptimalTransport(solver, filter_op=filter_op, parallel_wrap_shot=pwrap, transform_mode='quadratic', otq_add=1e-4)
            # objective = TemporalLeastSquares(solver, filter_op=filter_op, parallel_wrap_shot=pwrap, normalize_trace=False)
           ...
           # Proj_Op = JointPrj(Proj_Op1, Boxproj)
            ...
            invalg = PQN(objective, proj_op=Proj_Op, memory_length=10, maxiter_PGD=1000, PQN_start_iteration=10)
            ...
            result = invalg(shots, initial_value, nsteps_OTQ,
            ...
            if rank == 0:
                write_data('./objective_value_OTQ.mat', obj_vals_i, 0, 1, len(obj_vals_i))
            os.chdir('../')
        ...
        if rank == 0:
            ...
            write_data(ExpDir + '/objective_value_OTQ.mat', obj_vals, [0, 0], [1, 1], np.shape(obj_vals))
            write_data(WaveDir + '/objective_value_OTQ.mat', obj_vals, [0, 0], [1, 1], np.shape(obj_vals))
            ...
            for i_freq in range(n_fband):
                dir = './xm' + str(i_freq+1)+'_OTQ'
            
                for k in range(nsteps_OTQ):
