...
__all__ = ['generate_seismic_data', 'generate_seismic_data_from_file',
           'generate_shot_data_time', 'generate_shot_data_frequency',
           'generate_seismic_linearized_data', 'generate_shot_linearized_data_frequency']
    ...
    **kwargs : dict, optional
    ...
    if solver.supports['equation_dynamics'] == "time":
        ...
        if frequencies is None:
            ...
            generate_shot_data_frequency_list(
                shots, solver, model, frequencies, verbose=verbose, **kwargs)
        else:
            for shot in shots:
        ...
        if save_method is not None:
            if save_method == 'pickle':
                ...
                for i in range(len(shots)):
                    ...
                    f = h5py.File(fshot, "w")
                    dts = f.create_dataset(
                        fshot, shots[i].receivers.data.shape, shots[i].receivers.data.dtype)
                    dts[...] = shots[i].receivers.data
                    f.close()
                    fts = newpath+'/ts_'+str(i+1)+'.hdf5'
                    f = h5py.File(fts, 'w')
                    dts = f.create_dataset(
                        fts, shots[i].receivers.ts.shape, shots[i].receivers.ts.dtype)
        ...
        if save_method is not None:
            if save_method == 'pickle':
                ...
                raise NotImplementedError(
                    'h5py is not efficient with frequency data please use another data container')
            else:
                raise TypeError('Unknown save_method')
    if verbose:
...
def generate_seismic_linearized_data(shots, solver, model, model_perturbation, verbose=False, frequencies=None, **kwargs):
    """
    Given a list of shots and a solver, generates linearized seismic data.
    ...
    **kwargs : dict, optional
    ...
    if solver.supports['equation_dynamics'] == "time":
        generate_shot_linearized_data_time(shots, solver, model, model_perturbation, **kwargs)

    elif solver.supports['equation_dynamics'] == "frequency":
        if frequencies is None:
            raise TypeError('A frequency solver is passed, but no frequencies are given')
        else:
            generate_shot_linearized_data_frequency(shots, solver, model, model_perturbation, frequencies, **kwargs)
    else:
        raise TypeError("A time or frequency solver must be specified.")


def generate_seismic_extend_linearized_data(shots, solver, model, model_perturbation_extend, max_sub_offset,
                                            h, verbose=False, frequencies=None, **kwargs):
    """
    Given a list of shots and a solver, generates extended linearized seismic data.
    ...
    solver : pysit.WaveSolver
        Instance of wave solver to be used.
    model : background model parameters
    model_perturbation_extend : extended model perturbation
    max_sub_offset : maximum subsurface offset
    ...
    **kwargs : dict, optional
    ...
    if solver.supports['equation_dynamics'] == "time":
        generate_shot_linearized_data_time_extend(shots, solver, model, model_perturbation_extend, 
                                                  max_sub_offset, h, **kwargs)

    elif solver.supports['equation_dynamics'] == "frequency":
        if frequencies is None:
            raise TypeError(
                'A frequency solver is passed, but no frequencies are given')
        else:
            generate_shot_linearized_data_frequency_extend(shots, solver, model, model_perturbation_extend, 
                                                           max_sub_offset, h, frequencies, **kwargs)
    else:
        raise TypeError("A time or frequency solver must be specified.")
            ...
            if save_method == 'pickle':
                ...
                if os.path.exists(path):
                    try:
                        ...
                        raise IOError(
                            'Files are corrupted or generated with different save_method argument')
                else:
                    raise ImportError(
                        'There is no shot directory, please relaunch generate_seismic_data with save_method argument')
                    ...
                    try:
                        if 'frequencies' in kwargs:
                            ...
                            raise AttributeError(
                                """No frequencies specified in generate_seismic_data_from_file arguments""")
                    except IOError:
                        raise IOError(
                            'Files are corrupted or generated with different save_method argument')
                else:
...
def generate_shot_linearized_data_frequency(shots, solver, model, model_perturbation, frequencies, **kwargs):
    ...
    **kwargs : dict, optional
    ...
    for shot in shots:
        retval = GenerateDataObj.linear_forward_model(shot, model, model_perturbation, frequencies, 'simdata', dWaveOp0=None)
        shot.receivers.data_dft = retval['simdata']


def generate_shot_linearized_data_time(shots, solver, model, model_perturbation, **kwargs):
    """
    ...
    **kwargs : dict, optional
    ...
    for shot in shots:
        retval = GenerateDataObj.linear_forward_model(shot, model, model_perturbation, 'simdata', dWaveOp0=None)
        shot.receivers.data = retval['simdata']


def generate_shot_linearized_data_frequency_extend(shots, solver, model, model_perturbation_extend, max_sub_offset, h, frequencies, **kwargs):
    """
    Given a list of shots and a solver, generates extended linearized seismic data in the frequency domain. This function will use the function 
    of the linear_forward_model function in the `frequency_modeling.py`
    ...
    **kwargs : dict, optional
    ...
    for shot in shots:
        retval = GenerateDataObj.linear_forward_model_extend(shot, model, model_perturbation_extend, h, frequencies, 'simdata', dWaveOp0=None)
        shot.receivers.data_dft = retval['simdata']


def generate_shot_linearized_data_time_extend(shots, solver, model, model_perturbation_extend, max_sub_offset, h, **kwargs):
    """
    Given a list of shots and a solver, generates extended linearized seismic data in the time domain. This function will use the function 
    of the linear_forward_model function in the `temporal_modeling.py`
